...
zcat = Table.read("../catalogues/hlsp_candels_hst_wfc3_egs_v1_photoz_cat.fits").to_pandas()

cat.index = cat["ID"].astype(str).values
zcat.index = zcat["ID"].astype(str).values

zcat = zcat.groupby(cat["FLAGS"] == 0).get_group(True)
cat = cat.groupby(cat["FLAGS"] == 0).get_group(True)
print(cat.shape, zcat.shape)

IDs = cat.index
redshifts = zcat["Photo_z_Median"]

fit_instructions = get_fit_instructions()

fit_cat = pipes.fit_catalogue(IDs, fit_instructions, load_egs,
                              run="egs_v1_fix_redshifts",
                              cat_filt_list=filt_list, vary_filt_list=False,
                              make_plots=False, time_calls=False,
                              spectrum_exists=False, full_catalogue=True)

fit_cat.fit(n_live=500, verbose=False, mpi_serial=True)
